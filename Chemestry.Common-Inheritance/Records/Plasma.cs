...
public record Plasma : Substance
{
    ...
    public override string GetShape() => $"{SubstanceName} ({State}): doesn’t have a shape or volume";
    public override string ParticleMovement() => $"{SubstanceName} ({State}):particles are spread out and move randomly; contain some free ions and electrons, which gives plasma its ability to conduct electricity";
